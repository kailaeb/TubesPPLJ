...
class LoginPage(AuthPage):
    """
    Frontend UI for the login page.
    This version is corrected to work with the streamlined LoginBackend.
    """
    ...
    def setup_backend_connections(self):
        """Connect the necessary backend signals to frontend handlers."""
        # The backend now only has these two primary signals
        self.backend.login_response.connect(self.on_login_response)
        self.backend.error_occurred.connect(self.on_backend_error)
    ...
    def setup_form(self):
        ...
        title = QLabel("Login")
        title.setAlignment(Qt.AlignCenter)
        title.setStyleSheet("font-size: 36px; font-weight: bold; color: #2C2C2C; padding: 15px 0px;")
        title.setFixedHeight(70)
        form_layout.addWidget(title)
        ...
        username_label = QLabel("Username")
        username_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #2C2C2C;")
        ...
        password_label = QLabel("Password")
        password_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #2C2C2C;")
        ...
        no_account_label = QLabel("Don't have an account?")
        no_account_label.setAlignment(Qt.AlignCenter)
        no_account_label.setStyleSheet("font-size: 14px; color: #2C2C2C; font-style: italic; padding-top: 7px; margin-bottom: 5px;")
    ...
    @pyqtSlot(dict)
    def on_login_response(self, response):
        """Handles the login response from the backend."""
        self.hide_loading_state()
        
        status = response.get("status", "error")
        message = response.get("message", "Unknown response from server")
    ...
    @pyqtSlot(str)
    def on_backend_error(self, error_message):
        """Handles any generic errors from the backend."""
        self.hide_loading_state()
        self.show_error_message(error_message)
    
    def show_loading_state(self):
        """Disables the form and shows a 'Logging in...' message."""
        self.login_btn.setEnabled(False)
        self.login_btn.setText("Logging in...")
        self.username_input.setEnabled(False)
        self.password_input.setEnabled(False)
    ...
    def hide_loading_state(self):
        """Restores the form to its normal, enabled state."""
        self.login_btn.setEnabled(True)
        self.login_btn.setText("Login")
        self.username_input.setEnabled(True)
        self.password_input.setEnabled(True)
    ...
    def show_error_message(self, message):
        ...
        popup_layout = QVBoxLayout(self.error_popup)
        error_label = QLabel(f"⚠ {message}")
        error_label.setStyleSheet("color: #C0392B; font-size: 14px; font-weight: bold; border: none;")
        popup_layout.addWidget(error_label)
    ...
    def hide_error_message(self):
        """Hides the error popup if it exists."""
        if self.error_popup:
            self.error_popup.hide()
            self.error_popup.deleteLater()
            self.error_popup = None