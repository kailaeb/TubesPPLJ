    ...
    async def add_friend(self):
        ...
        try:
            ...
            async with websockets.connect(
                ...
                if auth_data.get("type") != "auth_success":
                    error_msg = auth_data.get("error", "Authentication failed")
                    self.add_friend_result.emit(False, {}, f"WSS Auth failed: {error_msg}")
                    return

                # Send add friend request via WebSocket
                add_friend_message = {
                    "type": "add_friend",
                    "username": self.username
                }
                ...
                response = await websocket.recv()
                response_data = json.loads(response)
                print(f"📨 Add friend response: {response_data}")
                ...
                if response_data.get("type") == "add_friend_response":
                    ...
                    print(f"❌ WSS Add Friend: Invalid response type - {response_data}")
                    self.add_friend_result.emit(False, {}, "Invalid response from server")
