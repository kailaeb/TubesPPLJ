...
class FriendListBackend(QObject):
    ...
    def handle_previous_conversations(self, message):
        """Handle previous conversations metadata and forward to home page"""
        print("📚 [FRIEND_LIST] Processing previous_conversations...")
        
        # The server sends conversations as a dictionary: {room_id: [messages]}
        conversations_data = message.get("conversations", {})
        
        if not isinstance(conversations_data, dict):
            print(f"❌ [FRIEND_LIST] Expected a dictionary for conversations, but got {type(conversations_data)}")
            return

        print(f"📚 [FRIEND_LIST] Found {len(conversations_data)} conversation rooms")
        
        # Process conversations metadata
        friend_conversations = {}
        
        # FIX: Iterate over dictionary items (room_id, messages) instead of just keys
        for room_id, messages in conversations_data.items():
            if not messages:
                continue

            # Determine the friend's username from the conversation
            # This logic assumes a 1-on-1 chat.
            # You should replace 'indira123' with the actual current user's username.
            current_username = "indira123" # Placeholder, this should be dynamic
            participants = {msg.get("sender_username") for msg in messages}
            participants.discard(current_username)
            
            friend_username = participants.pop() if participants else "Unknown Friend"

            # Use the timestamp of the last message
            last_message_time = messages[-1].get("timestamp")
            
            print(f"📚 [FRIEND_LIST] Processing conversation:")
            print(f"📚 [FRIEND_LIST]   room_id: {room_id}")
            print(f"📚 [FRIEND_LIST]   friend_username: {friend_username}")
            print(f"📚 [FRIEND_LIST]   last_message_time: {last_message_time}")
            
            if friend_username not in friend_conversations:
                friend_conversations[friend_username] = []
            
            # Create a placeholder entry for the chat list
            conversation_entry = {
                "sender": friend_username,
                "message": f"Previous conversation available",
                "timestamp": self.format_timestamp(last_message_time),
                "is_sent": False,
                "room_id": room_id
            }
            
            friend_conversations[friend_username].append(conversation_entry)
            print(f"📚 [FRIEND_LIST] Created conversation entry for {friend_username}")
        
        print(f"📚 [FRIEND_LIST] Processed {len(friend_conversations)} friend conversations")
        
        # Emit signal to forward to home page
        print("📤 [FRIEND_LIST] Emitting conversations_received signal...")
        self.conversations_received.emit(friend_conversations)
        print("✅ [FRIEND_LIST] Conversations signal emitted")
