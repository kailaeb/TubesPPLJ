...
class HomePageBackend(QObject):
    ...
    def handle_message_received(self, message_data):
        ...
        msg_type = message_obj.get("message_type", "text")
        sender = message_obj.get("sender_username", "Unknown")
        
        if msg_type == "image":
            print(f"📷 Received image from {sender}")
        elif msg_type == "file":
            file_name = message_obj.get("file_name", "Unknown file")
            print(f"📎 Received file from {sender}: {file_name}")
        else:
            content = message_obj.get("content", "")
            print(f"💬 Received text from {sender}: {content[:50]}...")
    ...
    def send_message(self, recipient_id: str, message_text: str):
        """Send a text message to another user by recipient_id."""
        ...
        print(f"📤 Sending text message to {recipient_id}: {message_text[:50]}...")
        self.websocket_thread.send_message(message_data)
    
    def send_file_message(self, recipient_id: str, file_name: str, file_data: str, 
                         file_size: int, mime_type: str, message_type: str = "file"):
        """Send a file or image message to another user."""
        if not self.is_connected:
            self.error_occurred.emit("Not connected to server")
            return
        
        if not recipient_id.strip():
            self.error_occurred.emit("Recipient ID cannot be empty")
            return
        
        if not file_data:
            self.error_occurred.emit("File data cannot be empty")
            return
        
        # Validate message type
        if message_type not in ["file", "image"]:
            self.error_occurred.emit("Invalid message type. Must be 'file' or 'image'")
            return
        
        message_data = {
            "recipient_id": recipient_id,
            "message_type": message_type,
            "message": file_name,  # Use file name as message content
            "file_name": file_name,
        ...
        print(f"📤 Sending {message_type} to {recipient_id}:")
    ...
    def send_image(self, recipient_id: str, file_name: str, file_data: str, 
                   file_size: int, mime_type: str):
        """Send an image message to another user."""
        print(f"📷 Preparing to send image: {file_name}")
        self.send_file_message(recipient_id, file_name, file_data, file_size, mime_type, "image")
    
    def send_file(self, recipient_id: str, file_name: str, file_data: str, 
                  file_size: int, mime_type: str):
        """Send a file message to another user."""
        print(f"📎 Preparing to send file: {file_name}")
        self.send_file_message(recipient_id, file_name, file_data, file_size, mime_type, "file")
...
class AuthenticatedWebSocketThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.loop = loop  # Store loop reference for message sending
                ...
                try:
                    ...
                    if self.websocket and not self.websocket.closed:
                        ...
                        if msg_type == "text":
                            ...
                            print(f"📤 Sent text to {recipient}: {content[:30]}...")
                        elif msg_type in ["image", "file"]:
                            file_name = message.get("file_name", "unknown")
                            file_size = message.get("file_size", 0)
                            print(f"📤 Sent {msg_type} to {recipient}: {file_name} ({file_size} bytes)")
                        else:
                            print(f"📤 Sent message type: {msg_type}")
            ...
            while not self.should_stop and self.websocket:
                try:
                    ...
                    if message_type == "new_message":
                        ...
                        msg_content_type = msg_obj.get("message_type", "text")
                        sender = msg_obj.get("sender_username", "unknown")
                        
                        if msg_content_type == "image":
                            print(f"📨 Received image from {sender}")
                        elif msg_content_type == "file":
                            file_name = msg_obj.get("file_name", "unknown")
                            print(f"📨 Received file from {sender}: {file_name}")
                        else:
                            content = msg_obj.get("content", "")
                            print(f"📨 Received text from {sender}: {content[:30]}...")
                        
                        self.message_received.emit(message_data)
                    else:
                        print(f"📨 Received response: {message_type}")
    ...
    def send_message(self, message_data):
        ...
        if not hasattr(self, 'loop') or not self.loop or self.loop.is_closed():
            print("❌ Loop not available for sending message")
            return
