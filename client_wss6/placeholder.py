...
class WebSocketThread(QThread):
    ...
    message_received = pyqtSignal(str, str, str)  # from_username, message, timestamp
    ...
    async def handle_new_message(self, data):
        ...
        self.message_received.emit(from_username, message_text, display_time)
    ...
    def process_message(self, msg, current_username):
        ...
        try:
            ...
            logger.info(f"📚 [WS] Processed: sender={sender}, content={content[:30]}..., is_sent={is_sent}")
...
class HomePage(QWidget):
    ...
    def handle_incoming_message(self, from_username, message_text, timestamp):
        """Handle incoming message"""
        print(f"📥 Message from {from_username}: {message_text}")
    ...
    # Add friend to chat list if not there
    # Only add if the sender is NOT the current user (don't add yourself to your own chat list)
        if from_username != current_username:
            print(f"📝 [HOME] add_friend_to_chat_list called for: {from_username}")
            ...
            print(f"📝 [HOME] Skipping add_friend_to_chat_list - sender is current user: {from_username}")
    ...
    # If viewing this chat, add message to display
        if self.current_chat_user == from_username:
            ...
            self.add_message_to_display(message_text, False, timestamp)
        else:
            print(f"📱 [HOME] Not currently viewing chat with {from_username} (current: {self.current_chat_user})")
    
    # Update chat list preview
        self.update_chat_list_preview(from_username, message_text, timestamp)
    ...
    # Ensure the recipient is in the chat list (this is the key fix)
    ...
    # Add to display
        self.add_message_to_display(message_text, True, timestamp)
    ...
    def display_chat_messages(self, friend_username):
        """Display chat messages for friend"""
