...
async def get_user_previous_conversations(user_id):
    """Return actual conversation data with messages for the user"""
    try:
        ...
        print(f"📚 [SERVER] Total conversations in database: {len(conversations)}")
        
        # Filter conversations where user is participant and return actual messages
        user_conversations = {}
        
        for room_id, messages in conversations.items():
            if not messages:
                print(f"📚 [SERVER] Skipping empty room: {room_id}")
                continue
            
            print(f"📚 [SERVER] Checking room {room_id} with {len(messages)} messages")
            ...
            for msg in messages:
                ...
                if sender_id == user_id or recipient_id == user_id:
                    ...
                    print(f"📚 [SERVER] User {user_id} is participant in room {room_id}")
                    break
            
            if user_is_participant:
                # Include this entire conversation with ALL messages
                user_conversations[room_id] = messages
                print(f"📚 [SERVER] Added room {room_id} with {len(messages)} messages to user conversations")
                
                # Debug: Show sample message structure
                if messages:
                    sample_msg = messages[0]
                    print(f"📚 [SERVER] Sample message structure: {sample_msg}")
        ...
        for room_id, msgs in user_conversations.items():
            ...
            if msgs:
                first_msg = msgs[0]
                print(f"📚 [SERVER]     First message: '{first_msg.get('content', '')}' from {first_msg.get('sender_username', '')}")
...
# async def get_user_previous_conversations(user_id):
#     """Get list of rooms/users that this user has chatted with"""
#     try:
#         async with chat_log_lock:
#             chat_data = load_chat_log()
#             conversations = chat_data.get("conversations", {})
...
#             for room_id, messages in conversations.items():
#                 # Check if user participated in this room
...
#                     # Extract other participant from room_id (assuming format like "user1_user2")
#                     try:
#                         # You might need to adjust this based on your room_id format
#                         other_user = room_id.replace(f"{user_id}_", "").replace(f"_{user_id}", "")
#                         if other_user != str(user_id):
...
#                             })
#                     except:
#                         # If room_id format is different, add as is
#                         user_conversations.append({
#                             "room_id": room_id,
#                             "last_message_time": last_message_time
#                         })
...
#             user_conversations.sort(key=lambda x: x.get("last_message_time", ""), reverse=True)
...
async def handle_new_message_save(user_info, data):
    ...
    try:
        ...
        # FIXED: Get existing room or create new one (instead of always creating new)
        ...
        async for msg in ws:
            if msg.type == WSMsgType.TEXT:
                try:
                    ...
                    recipient_id = data.get("recipient_id")  # <- the recipient's ID
                    message_content = data.get("message")    # <- the message's text

                    if recipient_id and message_content:
                        print(f"💬 [WS] Processing chat message from {user_info['username']} to recipient_id {recipient_id}")
                        try:
...
def main():
    ...
    print("🔐 Authenticated Cross-Protocol Message Bridge Server with Chat")
    print("=" * 60)
    ...
    if not ssl_context:
        ...
        try:
            ...
            print("\n✅ HTTP Authenticated Bridge Server with Chat starting...")
    ...
    print("\n🔐 Starting HTTPS + WebSocket Authenticated Bridge server with Chat")
    ...
    try:
        ...
        print("\n✅ HTTPS Authenticated Bridge Server with Chat starting...")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\n🛑 Authenticated bridge server with chat stopped by user")
